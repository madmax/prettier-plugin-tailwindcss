import { a as __toESM, t as __commonJSMin } from "./chunk-DYuqS7OD.mjs";
import { a as warn, i as error, n as sortClasses, t as sortClassList } from "./sorting-CRDUPFgk.mjs";
import { a as expiringMap, n as maybeResolve, t as loadIfExists } from "./resolve-CCCMDi-y.mjs";
import { i as visit, n as cacheForDirs, r as spliceChangesIntoString } from "./utils-D8dQkKEd.mjs";
import { getTailwindConfig as getTailwindConfig$1 } from "./sorter.mjs";
import { n as languages } from "./html-erb-kOWeSlrL.mjs";
...
function rewriteHtmlErb(source, env) {
	let changes = [];
	let i = 0;
...
function scanMarkup(source, start, env, changes) {
...
function scanStartTag(source, start, env, changes) {
	let i = start + 1;
	...
	while (i < source.length) {
		...
		if (next === null || next.fail || next.resume !== void 0) return source.length;
		i = next.index;
	}
	return source.length;
}
function scanAttribute(source, start, env, local) {
	if (!isAttrNameChar(source[start] ?? "")) return {
		index: start,
		fail: true
	};
...
function scanQuotedValue(source, quoteIndex, quote, isClass, env, local) {
	let i = quoteIndex + 1;
	let staticStart = i;
	let hasErb = false;
	let segments = [];
...
function sortClassSegments(source, segments, hasErb, env, local) {
	for (let index = 0; index < segments.length; index++) {
		...
		let sorted = sortClasses(value, {
			...
			collapseWhitespace: hasErb ? false : void 0
		});
		if (sorted !== value) local.push({
			start,
			end,
			before: value,
			after: sorted
...
function sortStandaloneErb(source, start, end, env, changes) {
	...
	for (let change of rubyChanges) changes.push({
...
function scanRubyClassStrings(body, env) {
	let changes = [];
	let i = 0;
	while (i < body.length) {
		let char = body[i] ?? "";
		...
		if (body.startsWith("<<", i) || isLineKeyword(body, i, "=begin") || isLineKeyword(body, i, "__END__")) return null;
...
function trySortClassLabel(body, start, env, changes) {
...
function scanRubyString(body, start) {
	let quote = body[start];
	let i = start + 1;
	while (i < body.length) {
		let char = body[i] ?? "";
		if (char === "\\") return null;
		if (quote === "\"" && char === "#" && (body[i + 1] === "{" || body[i + 1] === "@" || body[i + 1] === "$")) return null;
		if (char === quote) return i + 1;
		i += 1;
	}
	return null;
}
function isClassLabel(body, i) {
	if (body.slice(i, i + 6) !== "class:") return false;
	if (i === 0) return true;
	let prev = body[i - 1] ?? "";
	return isWs(prev) || prev === "(" || prev === "{" || prev === "[" || prev === "," || prev === ";" || prev === "|";
}
function isSafeClassTerminator(body, i) {
	if (i >= body.length) return true;
	let char = body[i] ?? "";
	if (char === "," || char === ")" || char === "}" || char === "]" || char === ";" || char === "#") return true;
	return isKeyword(body, i, "do") || isKeyword(body, i, "end");
}
function skipRawBody(source, start, tagName) {
	...
	while (i < source.length) {
		...
		if (source.slice(i, i + needle.length).toLowerCase() === needle) {
			...
			if (after === "" || isWs(after) || after === ">" || after === "/") return skipUntil(source, i + needle.length, ">");
		}
		i += 1;
	}
	return source.length;
...
function isErbComment(source, start) {
	let i = start + 2;
	if (source[i] === "-") i += 1;
	return source[i] === "#";
}
function isLineKeyword(body, i, word) {
	if (i > 0 && body[i - 1] !== "\n" && body[i - 1] !== "\r") return false;
	return isKeyword(body, i, word);
}
function isKeyword(body, i, word) {
...
function isAttrNameChar(char) {
	return char !== "" && !isWs(char) && char !== "=" && char !== ">" && char !== "/" && char !== "<" && char !== "\"" && char !== "'";
}
...
function isWsOnly(value) {
	for (let char of value) if (!isWs(char)) return false;
	return true;
}
...
const { parsers, printers } = createPlugin([
	...
	defineTransform({
		load: [{
			name: "html-erb",
			importer: () => import("./html-erb-kOWeSlrL.mjs").then((n) => n.t)
		}],
		parsers: { "html-erb": {} },
		printers: { "html-erb-text": {} },
