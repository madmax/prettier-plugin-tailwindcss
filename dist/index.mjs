...
function transformHtmlErb(ast, env) {
	ast.text = ast.text.replace(/\bclass="([^"]+?)( ?<|")/g, function(_fullMatch, classes, ending) {
		return `class="${sortClasses(classes, { env })}${ending}`;
	});
	ast.text = ast.text.replace(/\bclass:\s+(["'])([^"]+?)( ?#\{|["'])/g, function(_fullMatch, beginning, classes, ending) {
		return `class: ${beginning}${sortClasses(classes, { env })}${ending}`;
	});
}
